...
## 2026-09-24 — Portable Backlog Skills

- User instruction: “Add them to this repository as well so that other people or I can use them on other machines.” This followed the proposal to package bkl-sh and backlog-html-review with supporting files, setup instructions, and PM v2 compatibility checks.
- Implementation: Added both complete skill packages under `skills/`, a non-overwriting project-local installer, direct-use instructions, and regression tests. The summary instructions cover multiple Now tasks and typed prerequisites. The HTML exporter excludes fenced examples/comments, includes both plan types, separates formulation from plan approval, and reads coordination for open epics.
- Portability: The HTML generator uses Python 3.10+ standard-library modules and resolves no personal skill paths. Source packages remain under skills/ because this session denied writes to the repository's .agents directory even after an elevated retry. The installer targets a chosen project's .agents/skills directory; direct invocation needs no installation. No global skills or reference-project files were changed.
- Verification: Both skill manifests passed the skill-creator validator. The documentation check, all governance-tool unit tests, and generated JavaScript syntax check passed. Tests cover empty-template exports, multiple started tasks, authority display, source preservation, duplicate records, escaping, destination protection, and installation/running from a fresh temporary copy without Git.
- Verification limit: Real-browser verification remains unverified. The installed Chrome process exited before page load in both ordinary and elevated attempts, and no in-app browser execution tool was available. Keyboard operation, visual layout, browser persistence, clipboard delivery, and OneNote pasting are not claimed as passed. The generator and installation checks do not establish those behaviors.
- Bootstrap: Project task, epic, idea, plan, and log registries remain empty. Generated snapshots are disposable outputs and were not added as live project artifacts.
